  ...
  if(c->associativity == 1)
    {
      ...
      c->blocks[index]->valid = true; //Set the valid bit @ the index
      c->blocks[index]->tag = tag; //Set the tag bits @ the index

      //If write occurred 
      if(write)
	{
	  c->blocks[index]->dirty = true; //Set the dirty bit
	}
    }

  //The cache isn't direct mapped so quit with an error message
  else
    {
      perror("Cache associativity must equal 1\n");
      exit(-1);
    }
}
